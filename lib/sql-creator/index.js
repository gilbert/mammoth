...
var getTables = function (tables) {
    return Object.keys(tables).map(function (tableName) { return tables[tableName]; });
};
...
var getColumns = function (table) {
    return Object.keys(table.columns).map(function (columnName) { return table.columns[columnName]; });
};
...
var isEqualTable = function (fromTable, toTable) {
    return Object.keys(fromTable.columns).length === Object.keys(toTable.columns).length &&
        getColumns(fromTable).every(function (fromColumn) {
...
exports.createSql = function (from, to, shouldGenerateNames) {
        ...
        if (!fromTable) {
            ...
            if (tables.length === 1) {
                ...
                var query = "CREATE TABLE " + toTable.name + " (\n" + Object.keys(toTable.columns)
                    .map(function (columnName) { return toTable.columns[columnName]; })
                    .map(function (column) { return "" + tab + getColumnSql(toTable, column); })
                    .join(",\n") + "\n)";
                from.simulateQuery(query);
                queries.push({
                    name: toTable.name,
                    ...
                    dependencies: toTable.indexes
                        .filter(function (index) { return index.type === 'foreignKey'; })
                        .map(function (index) { return index.tableName; }),
                });
                if (shouldGenerateNames) {
            ...
            toTable.indexes.forEach(function (toIndex) {
                ...
                if (!fromIndex) {
                    if (toIndex.type === "primaryKey") {
                        ...
                        var query = "ALTER TABLE " + toTable.name + " ADD CONSTRAINT " + toIndex.name + " FOREIGN KEY (" + toIndex.columns.join(", ") + ") REFERENCES " + toIndex.tableName + (toIndex.referenceColumns.length > 0
        ...
        if (!toTable) {
            ...
            queries.push({
                ...
                dependencies: getTables(from.tables)
                    ...
                    return table.indexes.find(function (index) { return index.type === 'foreignKey' && index.tableName === fromTable.name; });
                })
    ...
    dropQueries.forEach(function (dropQuery) { return from.simulateQuery(dropQuery); });
    getTypes(to.types).forEach(function (toType) {
        var fromType = from.types[toType.name];
        if (!fromType) {
            var query = "CREATE TYPE " + toType.name + " AS ENUM (" + toType.labels
    ...
    return {
        queries: queries
            ...
            .map(function (query) { return query.text; }),
        names: names,
    };
};
